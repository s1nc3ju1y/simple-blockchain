...
class ProcessMessages(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        elif command == "send_tx":
            self.handle_sendtx(payload)
        elif command == "send_block":
    ...
    def handle_sendtx(self, payload):
        ...
        with self.server.node_manager.lock:
            ...
            for k in range(len(blockchain.current_transactions)):
                unconfirmed_tx = blockchain.current_transactions[-1 - k]
                if unconfirmed_tx.txid == new_tx.txid:
                    return
    ...
    def handle_version(self, payload):
        ...
        if version != 1:
            ...
            verack = Verack(1, int(time.time()), self.server.node_manager.node_id, client_node_id,
                            block_counts)
            self.server.node_manager.sendverack(new_node, verack)
    ...
    @staticmethod
    def add_to_candidate_blocks(blockchain, new_block):
...
class Node(object):
    ...
    @staticmethod
    def ping(sock, target_node_address, message):
        sock.sendto(zlib.compress(message), target_node_address)

    @staticmethod
    def pong(sock, target_node_address, message):
        sock.sendto(zlib.compress(message), target_node_address)

    @staticmethod
    def find_neighbors(sock, target_node_address, message):
        sock.sendto(zlib.compress(message), target_node_address)

    @staticmethod
    def found_neighbors(sock, target_node_address, message):
        sock.sendto(zlib.compress(message), target_node_address)

    @staticmethod
    def find_value(sock, target_node_address, message):
        sock.sendto(zlib.compress(message), target_node_address)

    @staticmethod
    def found_value(sock, target_node_address, message):
        sock.sendto(zlib.compress(message), target_node_address)

    @staticmethod
    def store(sock, target_node_address, message):
        sock.sendto(zlib.compress(message), target_node_address)

    @staticmethod
    def send_tx(sock, target_node_address, message):
        sock.sendto(zlib.compress(message), target_node_address)

    @staticmethod
    def send_block(sock, target_node_address, message):
        sock.sendto(zlib.compress(message), target_node_address)

    @staticmethod
    def send_version(sock, target_node_address, message):
        sock.sendto(zlib.compress(message), target_node_address)

    @staticmethod
    def sendverack(sock, target_node_address, message):
        sock.sendto(zlib.compress(message), target_node_address)
...
class NodeManager(object):
    ...
    def __init__(self, ip, port=0, genesis_node=False):
        self.ip = ip
        self.port = port
        ...
        self.blockchain = Blockchain(genesis_node)
        ...
        # 矿工线程
        self.minner_thread = threading.Thread(target=self.miner)
    ...
    def bootstrap(self, seed_nodes=None):
        """
        根据初始节点引导初始化
    ...
    def heartbeat(self, node, bucket, node_idx):
    ...
    def set_data(self, key, value):
        ...
        k_nearest_nodes = self.iterative_find_nodes(data_key)
    ...
    def sendtx(self, tx):
        ...
        for node in k_nearest_ndoes:
            ...
            msg_obj = packet.Message("send_tx", tx)
            msg_bytes = pickle.dumps(msg_obj)
            self.client.send_tx(self.server.socket, (node.ip, node.port), msg_bytes)
    ...
    def sendblock(self, block):
        ...
        for node in k_nearest_ndoes:
            ...
            msg_obj = packet.Message("send_block", block)
            msg_bytes = pickle.dumps(msg_obj)
            print('[Info] send block', node.ip, node.port, block.current_hash)
            self.client.send_block(self.server.socket, (node.ip, node.port), msg_bytes)

    def sendversion(self, node, version):
        msg_obj = packet.Message("version", version)
        msg_bytes = pickle.dumps(msg_obj)
        self.client.send_version(self.server.socket, (node.ip, node.port), msg_bytes)

    def sendverack(self, node, verack):
